...
class SuperSpotTakerStrategy(FundRateArbitrageTemplate):
    """
    现货挂taker单，合约maker单
    """
    author = "bitquant"
    ...
    variables = ["current_spread", "spot_trade_vol", "spot_quote_vol",  "future_vol", "liquid_price", "reduce_future_pos", "insufficient"]
    ...
    def on_tick(self, tick: TickData):
        ...
        # 如果没有行情数据就不会处理.
        if not self.spot_tick or not self.future_tick or not self.future_contract or not self.spot_contract or not self.premium_index_data:
            return

        if not self.spot_trade_account or not self.spot_quote_account or not self.future_position:
            return
        ...
        #     if delta_vol * self.spot_tick.bid_price_1 >= self.min_trade_amount:
        #
        #         vol = min(abs(delta_vol), self.spot_trade_account.balance)
        ...
        if self.liquid_price > 0 and abs(self.future_position.volume) >= self.future_contract.min_volume:
            ...
            if liquid < self.close_before_liquidation_pct:
                ...
                self.orders.extend(ids)
                print(f"合约下减仓单了，防止爆仓: {round(cover_price, 3)}@{vol}, orders: {self.orders}")
                return None


        # 处理合约的仓位和现货仓位相等.
        delta_vol = abs(self.future_position.volume) - self.spot_trade_account.balance
        ...
        if abs(delta_vol) * self.spot_tick.bid_price_1 >= self.min_trade_amount:  # and abs(delta_vol) >= self.future_contract.min_volume:
            ...
            if delta_vol > 0:
                ...
                vol = floor_to(delta_vol, self.spot_contract.min_volume)
                price = self.spot_tick.ask_price_1 * (1 + self.slippage_tolerance_pct / 100)

                if self.spot_quote_account.balance >= price * vol:
                    ids = self.buy(self.spot_vt_symbol,price
                                     , vol)
                    self.orders.extend(ids)
                    print(f"现货对冲做多：{self.spot_tick.ask_price_1}@{vol}, orders: {self.orders}")
                return None

            elif delta_vol < 0:

                vol = floor_to(abs(delta_vol), self.spot_contract.min_volume)

                ids = self.sell(self.spot_vt_symbol,
                                 self.spot_tick.bid_price_1 * (1 - self.slippage_tolerance_pct / 100),
                                 vol)
                self.orders.extend(ids)

                print(f"现货卖出：{self.future_tick.ask_price_1}@{vol}, orders: {self.orders}")

                return None

        ##############################################################

        if self.spot_tick.ask_price_1 <= 0:
            return

        # 比较卖价的价差.
        self.current_spread = round((self.future_tick.ask_price_1 / self.spot_tick.ask_price_1 - 1) * 100, 3)
        ...
        if self.current_spread <= self.close_spread_pct and self.current_rate <= self.close_rate_pct:
            ...
            trade_amount = abs(self.future_position.volume)
            print(
                f"价差和资金费率变小，让程序减仓. 价差:{self.current_spread} <= {self.close_spread_pct},  费率: {self.current_rate} <= {self.close_rate_pct}")

            trade_vol = floor_to(self.trade_max_usd_every_time / self.future_tick.ask_price_1,
                                 self.future_contract.min_volume)

            trade_vol = min(self.spot_tick.bid_volume_1, trade_vol, trade_amount)

            if trade_vol >= self.future_contract.min_volume:
                ids = self.cover(self.future_vt_symbol, self.future_tick.bid_price_1, trade_vol, limit_maker=True)
                self.orders.extend(ids)
                print(f"合约减仓：{self.spot_tick.ask_price_1}@{trade_vol}, orders: {self.orders}")
                self.close_pos = True
        ...
        short_amount = self.initial_target_pos - abs(self.future_position.volume)
        ...
        if short_amount > 0:
            ...
            if self.insufficient:
                self.write_log(f"合约保证金不够，没法继续开仓, 修改设置的开仓数量从:{self.initial_target_pos} => {abs(self.future_position.volume)}")
            ...
            if self.current_spread < self.open_spread_pct or self.current_rate < self.open_rate_pct:
                # print(
                #     f"价差和资金费率不满足开仓条件, 价差:{self.current_spread} < {self.open_spread_pct},  费率: {self.current_rate} < {self.open_rate_pct}")
                return None

            trade_value = min(self.spot_quote_account.balance, self.trade_max_usd_every_time)

            trade_vol = floor_to(trade_value / self.future_tick.ask_price_1, self.future_contract.min_volume)

            trade_vol = min(self.spot_tick.ask_volume_1, trade_vol, short_amount)

            if trade_vol * self.future_tick.ask_price_1 >= self.min_trade_amount and trade_vol >= self.future_contract.min_volume:
                ids = self.short(self.future_vt_symbol, self.future_tick.ask_price_1, trade_vol, limit_maker=True)
                self.orders.extend(ids)

                print(f"合约下做空单:{self.future_tick.ask_price_1}@{trade_vol}, orders: {self.orders}")

        elif short_amount < 0:
            # 减少仓位.
            if abs(short_amount) >= self.future_contract.min_volume:
                trade_vol = floor_to(self.trade_max_usd_every_time / self.future_tick.bid_price_1,
                                     self.future_contract.min_volume)

                vol = min(self.spot_tick.ask_volume_1, trade_vol, abs(short_amount), abs(self.future_position.volume))
                if vol >= self.future_contract.min_volume:

                    ids = self.cover(self.future_vt_symbol, self.future_tick.bid_price_1, vol, limit_maker=True)
                    self.orders.extend(ids)
                    self.close_pos = True  # 设置它是平仓.
                    print(f"合约下平仓单：{self.future_tick.bid_price_1}@{vol}, orders: {self.orders}")
            else:
                self.close_pos = False
