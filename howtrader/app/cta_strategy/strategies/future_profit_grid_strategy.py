...
class FutureProfitGridStrategy(CtaTemplate):
    ...
    parameters = ["grid_step", "profit_step", "trading_size", "max_pos_size", "profit_orders_counts",
    ...
    def process_timer_event(self, event: Event):
        ...
        if self.stop_order_interval >= STOP_TIMER_INTERVAL:
            ...
            # 如果仓位达到最大值的时候.
            if abs(self.position_calculator.pos) >= self.max_pos_size * self.trading_size:
    ...
    def on_order(self, order: OrderData):
        ...
        if order.status == Status.ALLTRADED:
            if order.vt_orderid in (self.long_orders + self.short_orders):
                ...
                if self.tick and abs(self.position_calculator.pos) < self.max_pos_size * self.trading_size:
                    buy_step = self.get_step()
                    sell_step = self.get_step()
